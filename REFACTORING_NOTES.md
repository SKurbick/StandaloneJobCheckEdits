...
| 6 | Один DB pool/context на job | Done | `Database1` вынесен в `db.py`; новый entrypoint держит один job-scoped DB context; `CardActualizationService.actualize_card_data_in_db` и legacy `ServiceGoogleSheet.add_new_data_from_table` используют переданный `db` вместо создания дополнительных pool. | Старый legacy `Service` и direct-run legacy entrypoint все еще содержат собственные `Database1()`, но новый `main.py`/`scheduler.py` их не использует. |
...
- 2026-07-05: DB pool уменьшен до настраиваемого standalone-дефолта `min=1/max=3`; новый entrypoint передает один job-scoped `db` через `check_edits_columns -> change_cards_and_tables_data -> add_new_data_from_table`, скрытое создание второго pool в этой ветке удалено.
